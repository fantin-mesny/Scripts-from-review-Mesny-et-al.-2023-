...
## pId to tId
gffFiles={f.split('_')[0]:f for f in os.listdir('Genomic_data/Sordariomycetes/gff') if f.split('_')[0] in sorda}
...
with open('Genomic_data/Sordariomycetes/cds/all.ffn','r') as inp:
	allt=SeqIO.to_dict(SeqIO.parse(inp, 'fasta'))
allt_renamed={t.replace('|'+t.split('|')[2]+'|','|'+t2p[t.split('|')[1]][t.split('|')[2]]+'|'):allt[t] for t in allt}


Dir='Genomic_data/Sordariomycetes/protein/OrthoFinder/Results/Orthogroup_Sequences/'
for f in os.listdir(Dir):
	...
	with open('Genomic_data/Sordariomycetes/protein/OrthoFinder/Results/Orthogroup_Sequences_Transcripts/'+f.replace('fa','ffn'),'w+') as outp:
